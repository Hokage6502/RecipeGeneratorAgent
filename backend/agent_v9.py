# Performace reduced more structured output but taking more time investigate why>??
# Best structured output after repetitive test performing good
        ...
        try:
            ...
            if isinstance(result.data, NoRecipeFound):
                ...
                for idx, (step, time) in enumerate(zip(recipe.steps, recipe.step_times)):
                    # Ensure no repetition of step number or time
                    # print(f"{idx+1}. {step} (Time: {time})")
                    # for idx, (step, time) in enumerate(zip(recipe.steps, recipe.step_times)):
                    # # Ensure no repetition of time formatting
                    # # Only format the time once, removing any extra "Time: " prefix
